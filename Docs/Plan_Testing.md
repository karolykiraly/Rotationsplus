...
**Bounded gap — real-token auth validation (recorded in slice 8, CIAM wiring).** Integration tests run under `TestAuthHandler`, which replaces both real JWT-bearer schemes with a header-driven principal. So the authz matrix and endpoint tests verify *role gating* thoroughly, but they do **not** exercise the real Entra path: the "Smart" issuer-routing scheme, JWT signature/issuer/audience validation, or the staff-vs-customer scheme boundary. Those are covered instead by (a) `AuthSchemeSelectorTests` (routing logic in isolation), (b) `RoleBoundaryTests` (staff/customer role-name disjointness — the invariant the boundary rests on), and (c) a manual DEV smoke test once the CIAM user-flow is live (a real customer token reaches `/api/customer/me`; a token with the wrong audience is 401). The next hardening — pinning each policy to its authenticating scheme — needs the factory to forward the real schemes to `TestAuthHandler`; tracked for the customer-portal slice.
