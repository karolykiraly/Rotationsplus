...
## Production parity is mandatory (2026-06-28)
The public site is part of the **port**, held to the same **≥99% production-faithful** bar as every other screen (CLAUDE.md **Prime Directive**, [[match-production-ui-first]]). Production (`Live_Code/06102026/rotationsplusweb-v4`) is **ground truth**; Figma is the visual aid; **production wins on conflict**. A fidelity audit on 2026-06-28 found the first public-site build had quietly *simplified* production (~88%, not ≥99%) — the corrective parity pass is tracked in `.claude/plans/sleepy-sprouting-dolphin.md`. **No missing features, no considerable UI changes; UX improvements deferred to post-cutover.**

**Per-page parity checklist** — each page must render *everything* its production source renders. Build from this inventory, then diff side-by-side against `Live_Code` before declaring done:
- **Landing (`/` ← `routes/NewHome.js` + `Home.js`):** hero search input + **Search** button; the **8 filter controls** in order — Specialty, Program Type, **Clinical Needs** (multi-select), **Ratings**, City, State, Duration, Pricing; results panel = Leaflet **map w/ clustering** + scrollable list with **Sort** dropdown + **Reset filters** link; anonymous gating (Search/filter/view → `/portal`); Our Benefits; How It Works; **testimonials carousel**; dual CTA; **Our Partners** logos.
- **For Preceptors (`routes/ForPreceptors.js`) / Consulting (`routes/ForConsultants.js`):** hero + benefits; the **5 image-paired onboarding blocks** (`onboard1–5`) with **desktop + mobile** image variants; CTA → `/portal`.
- **Resources (`routes/Resources.js`):** "Content types" row of 6 buttons (Webinar active, Articles/Video/Guide/Case Study/Report) — **decorative in production**; article cards each with **"Learn more"** button (decorative); "Show more" pagination (appears only >10 articles). *(The production 6th placeholder article "We don't have a 6th article yet" is a production defect — flagged to owner.)*
- **Footer (`components/Footer.js`):** logo; address **777 S Figueroa Ste 4600**; **2024 ©** copyright; nav (Home/Our Process/Our Team/For Preceptors/Consulting Services/Blog/FAQ); Privacy + Terms; **info@rotationsplus.org** + phone; **4 social icons** (Facebook, Instagram, YouTube, Reddit).
- **About / Our Process / Our Team / FAQ / Privacy / Terms:** content verbatim (audited 95–100%).
