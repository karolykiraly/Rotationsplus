...
| Route | Component | Purpose |
|---|---|---|
| `/admin/programs` | `AdminProgram.js` | Program list tabbed by type (InPerson/Research/Consultation/TeleRotation/TeleResearch), debounced search, filters, add/delete. **PR-4 (Production UI parity) aligned the list columns to the live screen:** the **Program Name** column derives `"{Specialty} Physician"` (the legacy default) so it's no longer a duplicate of the **Specialty** column; the **"Retail Amount"** column now shows the **weekly honorarium** (owner-confirmed 2026-06-26 to match production, which displays `weekly_honoarium` under that label even though a separate retail field exists). Backend: `ProgramSummaryResponse` gains a **staff-only nullable `WeeklyHonorarium`** — both `GET /api/programs` and `/catalog` (MarketplaceViewer; students can call them) **null it for customer callers**, mirroring the detail endpoint, so margin stays hidden. No migration (the column already existed). **PR-4b wired the Filter modal:** the live 6-dimension `FilterProgram` (program number · location · specialty · instant-approval · honorarium range · tags) is built (`FilterProgramModal`) and applied server-side via new optional `GET /api/programs` params (specialtyId/city/instantApproval/honorariumMin/Max/programNumber/tags[]); the filter button now opens it with an active-count badge. **Security:** the honorarium filter is **staff-gated** (`includeHonorarium`) so a customer can't probe `honorariumMin/Max` to infer the margin the response already nulls. The honorarium dual-range slider is rendered as min/max inputs; location is a free-text "City, State" box (contains-match). **PR (admin-programs-list-parity, 2026-06-30) brought the screen to ≥99% production parity:** (1) **Program Name is now a real field** — `ProgramName` added end-to-end (entity + `AddProgramName` migration nullable ≤200 + Summary/Detail responses + Create/Update requests + trim/length validation); the list shows the stored name and only falls back to `"{Specialty} Physician"` when null, so a renamed program shows its real name (legacy `program_name`). (2) **The FilterProgram modal now matches the live controls exactly:** the location box became a **City/State dropdown** (shown only on the InPerson / InPersonResearch tabs, as production does) backed by a new **`GET /api/programs/locations`** distinct-locations endpoint (MarketplaceViewer; server pagination means the client can no longer derive the list); Specialty / Approval Type / Clinical Needs became **checkbox lists with "All"** (incl. the "100% Inpatient Hospitalist"→stored-id `Hospitalist` remap, 19 tags in exact legacy order); the Retail Amount filter gained a **dependency-free dual-range `RangeSlider`** (0–15,000) alongside the min/max inputs. (3) **Filters are per-tab** (legacy `programFilter0..4`) — each program-type tab keeps its own filter, so a city set on InPerson can't leak onto another tab. (4) Instant Approval renders uppercase **YES/NO**. |
| `/admin/programs/:slug` | `AdminProgramDetail.js` | Edit program: description, image, type, preceptor assignment, publish state |
| `/admin/rotations` | `AdminRotations.js` | Current + history tabs; full status filter (12 statuses); edit rotation modal (status/payment/dates/docs), change-date modal, delete. **PR-3 (Production UI parity) reworked this to the live layout:** two stacked **Current Rotations** / **Historical Rotations** sections (each server-paginated with its own search), the production columns **Rotation # · Preceptor · Student · Start–End · Retail Amount · Needs Visa ☑ · Status** (colour-coded, "NotStarted"→"Approved"), dropping the rewrite's Specialty/Type/Weeks columns + the status-filter dropdown + the inline Edit/Delete/Refund (owner decision 2026-06-26 — hide them to match production; the backend DELETE/refund endpoints remain). A **View** button opens the **Selected Rotation** detail panel (Replace program / Change dates / status dropdown / Save), saved via the single reliable update endpoint. **Backend:** `RotationSummaryResponse` +`RetailAmount` (program retail/wk × weeks) +`NeedsVisa` (correlated subquery — the booked student's `VisaStatus == NeedsVisaHelp`); a **`scope=current\|historical`** list param (Current = non-terminal Pending/NotStarted/Active/ToBeEvaluated, Historical = terminal Completed/Cancelled/Refunded/Abandoned/Rejected); `RotationDetailResponse` +`ProgramNumber` +`RetailAmount` (Rotation Cost) +`PaidAmount` (sum of Succeeded payments). No migration. **PR-3b wired the Filter modal:** the live `FilterRotation` (start/end date · amount range · needs-visa · status · rotation number) is built (`FilterRotationModal`) and applied server-side via new optional `GET /api/rotations` params (startFrom/endTo/retailMin/Max/needsVisa/rotationNumber); **one shared filter** narrows BOTH sections (legacy parity), with an active-count badge. The amount dual-range slider is rendered as min/max inputs. (PR-3b shipped alongside PR-4b's Programs filter in one "Filter modals" PR.) **PR (admin-rotations-list-parity, 2026-06-30, PR #90) closed the remaining list-cell fidelity gaps vs the production screenshot (frontend-only):** Start/End date renders **MM/DD/YY** in the Current section and **MM/DD/YYYY** in the Historical section joined by " to " (was a localized "Mon D, YYYY" with an en-dash; `formatDate` now slices the wire parts → also drops a latent `Date()` timezone shift); **Retail Amount** renders the raw **`$amount`** with no thousands separators (legacy `${amount}`); **Preceptor + Student names** render **bold blue** (`heading-xxxs` + `.rot-name`) matching production's profile-link styling — the click-through to the member-profile pages is **deferred** until those routes exist (Contacts) and flagged in-code; and the date/amount/status cells render in **PT Sans** (`.body-md`) not PT Serif. Status colour-coding already matched (Active→green, Pending→muted; the legacy yellow Onhold/Suspended states don't exist in the rewrite's status model). |
