...
> **Customer MSAL wiring caveat (from slice 8).** The SPA already carries the customer (CIAM) MSAL config — `customerMsalInstance` / `customerLoginRequest` / `customerApiScope` in `authConfig.ts` — but it is dormant; only the staff instance is wired in `main.tsx`. When this slice activates customer sign-in, two things must be handled or the staff and customer MSAL instances collide (both are `PublicClientApplication`s on the same origin + `sessionStorage`): (1) give the customer instance a **distinct `redirectUri`** (e.g. `/auth/customer`, registered on `rplus-web-ext` via `infra/ciam/Configure-Ciam.ps1 -SwaHostname …`) so redirect responses aren't consumed by the wrong instance; (2) make API token acquisition **issuer/instance-scoped** — today `api.ts` `getAllAccounts()[0]` is issuer-blind and would pick whichever account is first. Same caveat applies to `Plan_Preceptor.md`.
