...
- **MFA enforced for staff roles** (Admin/Sales/SDR) — **replaces the current homegrown 2FA entirely**: today (`AdminLogin.js` → `verifyAdmin`/`loginAdmin`) email+password triggers a Twilio SMS + email 4-digit code that the backend validates. In the rewrite, Entra performs the second factor — the custom code AND the Twilio dependency for login disappear (a security upgrade: policy-enforced, can't be bypassed). The dedicated staff URLs (`/rotationsplusadmin`, `/rotationsplussales`, `/rotationsplussdr`, `/rotationsplusinstitutions`) are kept as routes that trigger the Entra flow.
  - **Public-root restructure (2026-06-27, public-site program — `Plan_Public_Site.md`):** the SPA root `/` is now the **public, anonymous marketing site** (no MsalProvider). Consequently the workforce `redirectUri` moves from `/` (origin) to **`/admin`** (the route inside the staff MsalProvider that receives the login response); CIAM still redirects to `/portal`; staff sign-out lands on the public `/`. The `/rotationsplusadmin|sales|sdr` URLs are thin login launchers (fire `loginRedirect`, then role-route via `roleHome()`). **Requires registering the per-environment `/admin` SPA redirect URI on `rplus-web`.** The three router branches (public / staff-workforce / customer-CIAM) keep the no-root-MsalProvider invariant so the two instances never contend for an auth-response hash.
...
- **Authorization policies are scheme-pinned (PHASE 2e / SEC-1):** each named policy is bound to its authenticating JWT-bearer scheme — `StaffOnly`/`AdminOnly` → workforce, `CustomerOnly` → customer, `MarketplaceViewer` → both — so a token validated by the wrong directory can't satisfy a policy even if a role name were ever shared (defense-in-depth on top of the disjoint-role invariant). The integration `TestAuthHandler` is forwarded the real scheme names (`RotationsApiFactory` mutates `SchemeMap[...].HandlerType`) so pinned policies still authenticate the header-driven test identity.
- **API hardening (PHASE 2e):** rate limiting (`AddRateLimiter`) protects the anonymous webhook (per-IP fixed window) and the authenticated money paths — open-deposit + self-booking (per-user/oid); limits are config-bound (`RateLimiting:*`) with generous defaults. A uniform RFC 7807 **ProblemDetails** error contract (`AddProblemDetails` + `UseExceptionHandler`) replaces ad-hoc 500s, and rejected (429) requests carry a ProblemDetails body + `Retry-After`. **Deferred:** tightening `AllowedHosts` (DEV still uses Azure default hostnames; revisit with custom DNS at offboarding); real Stripe `Stripe-Signature` parse + replay-tolerance at the gateway swap (cutover checklist).
...
- **Clean schema redesign** (~30 tables): Strapi's `_links` join tables → real FKs; users-permissions tables → `users` + Entra; duplicate document tables (`document`, `sign_document`, `docu_sign`, `pal_document`, `inno_document`) → one `documents` table + `document_kind` + provider-specific detail tables only where needed; JSON columns (`detail`, `pal_info`, `expertise_area`, `privilege_hospitals`) → `jsonb` with typed POCO mapping where shape is stable.
  - **Documents subsystem (PHASE 2g) — being built MVP-first** in the `documents` schema: `document_types` (catalog, replaces 50+ free-text strings) + `program_required_documents` (per-program required-docs config, replaces the legacy `program.require_documents` JSON) + `rotation_documents` (per-rotation instance with an explicit `DocumentStatus` state machine — `UploadNeeded→Submitted→Approved|Rejected`,`Expired` — collapsing the legacy status soup). On booking, required docs are materialized per rotation; the student-facing "Documents" tracker column is the computed `NotRequired/Missing/Complete` state. **Deferred to later phases:** OCR-via-SFTP validation, HelloSign e-sign (removed vendor), PAL visa letters (paid add-on), deadline-reminder emails. The legacy `sign_document`/`docu_sign` tables are dead code, not carried over.
