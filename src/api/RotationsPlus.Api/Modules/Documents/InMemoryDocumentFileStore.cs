...
/// <summary>
/// In-memory stand-in for <see cref="IDocumentFileStore"/> used when no storage connection is
...
/// </summary>
public sealed class InMemoryDocumentFileStore : IDocumentFileStore
{
    ...
    public async Task<string> UploadAsync(Guid rotationDocumentId, Stream content, string contentType, CancellationToken cancellationToken)
    {
        ...
        var blobName = $"{rotationDocumentId}/{Guid.NewGuid():N}";
    ...
    public string? GetReadUrl(string? blobName) =>
        string.IsNullOrWhiteSpace(blobName) ? null : $"https://documents.local/{blobName}";
}
